...
{
	static int16_t Xoffset = -57;
	static int16_t Yoffset = -11;
	static int16_t Zoffset = -86;
	float Xh, Yh, Zh;
	float Xe, Ye;
	float th;
	Xh = mx - Xoffset;
...
//	Xe = Xh*cos(ay)+Yh*sin(ax)*sin(ay)-Zh*cos(ax)*sin(ay);
//	Ye = Yh*cos(ax)+ Zh*sin(ax);
	Ye = Xh*sin(p)*sin(r)+Yh*cos(p);
	Xe = Xh*cos(r);

	th = atan(Ye/Xe);
	th *= fRad2Deg;	
	//printf("Xe=%.2f\tYe=%.2f\tthk=%.2f\t", Xe, Ye, th);
	//转换到0~360度
	if(Xe < 0 && Ye > 0) th += 180;
